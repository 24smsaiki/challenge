                ...
                <table class="min-w-max w-full table-auto">
                    <thead>
                        <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                            <th v-for="column in props.columns" class="py-3 px-6 text-left">{{column}}</th>
                            <th class="py-3 px-6 text-left"></th>
                        </tr>
                    </thead>
                    <tbody class="text-gray-600 text-sm font-light">
                        <tr v-for="(row, index) in props.data" class="border-b border-gray-200 hover:bg-gray-100">
                            <td v-for="(value, index) in row" class="py-3 px-6 text-left whitespace-nowrap">
                                <div class="flex items-center">
                                    <template v-if="typeof value === 'string' && value.length > 190">
                                        <span>{{ value.slice(0, 190) + "..." }}</span>
                                    </template>
                                    <template v-if="(typeof value === 'number' && index !== 'id' && index !== 'state' && index !== 'status' && index !== 'orders' && index !== 'Address' )">
                                        <span>{{ value }} €</span>
                                    </template>
